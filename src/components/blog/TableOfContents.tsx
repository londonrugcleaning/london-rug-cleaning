...
export const TableOfContents = ({ content }: TableOfContentsProps) => {
  ...
  const headings = allHeadings.filter(h => h.level === 2 || h.level === 3);
            ...
            {headings.map((heading, index) => (
              <li key={`${heading.id}-${index}`}>
                <a
                  ...
                  className={`group flex items-center gap-3 py-2 rounded-lg text-sm text-slate-600 dark:text-slate-400 hover:text-blue-700 dark:hover:text-blue-400 hover:bg-blue-50/70 dark:hover:bg-blue-900/20 transition-all duration-200 ${heading.level === 3 ? 'pl-8 pr-3' : 'px-3'
                    }`}
                >
                  {heading.level === 2 && (
                    <span className="flex items-center justify-center w-6 h-6 rounded-md bg-slate-200/80 dark:bg-slate-700/60 text-xs font-medium text-slate-500 dark:text-slate-400 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200">
                      {allHeadings.filter(h => h.level === 2).findIndex(h => h.id === heading.id) + 1}
                    </span>
                  )}
                  {heading.level === 3 && (
                    <span className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-slate-500 group-hover:bg-blue-500 transition-colors duration-200" />
                  )}
                  <span className={heading.level === 2 ? 'font-medium' : 'font-normal text-slate-500 dark:text-slate-500'}>
                    {heading.text}
                  </span>
                </a>
              </li>
